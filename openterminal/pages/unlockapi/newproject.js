...
export default function Project() {
  ...
  const listItems = data.repos.filter(repo => {
    ...
    <div key={repo} style={{padding: '5px', borderRadius: '10px', border: 'none', margin: '5px', backgroundColor: 'rgb(235, 235, 235, 0.7)', display: 'flex', flexDirection: 'row'}}>
      ...
      <h3 style={{marginLeft: 'auto', marginRight: '7px'}}><span><Link href=""><a onClick={() => setContent(<h2><span className="grey">Connect </span>{data.user}/{repo}<br /><span style={{fontSize: '13px', fontWeight: '600'}}>Start a new project under https://openterminal.vercel.app/unlockapi/{data.user}/{repo}. Details can be changed below.</span></h2>)}>Connect <FontAwesomeIcon icon="arrow-right" /></a></Link></span></h3>
    </div>
  );
