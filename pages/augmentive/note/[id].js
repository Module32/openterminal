...
import { faStar, faShare, faXmark, faClipboard, faArrowLeft, faLink } from '@fortawesome/free-solid-svg-icons'
...
export default function Note({ dbuser, dbnote }) {
    ...
    const [ editability, setEditability ] = useState(dbnote ? dbnote.editability : 'view');
    const [ viewability, setViewability ] = useState(dbnote ? dbnote.viewability : 'private');
      ...
      <>
        <div className='flex pt-3 pb-2 px-5 bg-white font-medium'>
          ...
          <div className='ml-auto mr-1'>
            <Tippy
              content={<p>{star === true ? 'Unstar this note' : 'Star this note'}</p>}
              className='backdrop-blur-md bg-slate-200/80 p-1 px-2 font-medium shadow-xl border border-slate-400/75 rounded'>
        ...
        <div className='bg-white'>
          <Tiptap content={`Write something!`} formattingClass='mx-2' propsClass='min-h-screen' user={session && session.user.name} />
        </div>
        ...
        {shareModal === true && <div className='fixed flex h-screen top-0 m-0 rounded-none hover:bg-black/30 w-screen bg-black/30 items-center justify-center cursor-default'>
          <div className={`bg-sky-400 font-medium lg:w-5/12 md:w-7/12 sm:w-10/12 ${isMobile && 'w-11/12'} rounded border border-slate-400`}>
            <div className='py-2 px-5 text-white text-left'>
              <h1 className='text-3xl font-semibold flex items-center'>Share your note <button className='ml-auto p-1 px-3 bg-sky-500 text-2xl border-none hover:bg-sky-600/50 text-white' onClick={() => setShareModal(false)}><FontAwesomeIcon icon={faXmark} /></button></h1>
            </div>
            <div className='p-5 bg-slate-100'>
              <p className='text-xl'>Invite specific users...</p>
              <input className='w-full p-1.5 px-2 ml-0' type='email' placeholder='Search by email, press Enter to add'></input>
              <hr className='my-3' />
              <p className='text-xl'>...Or share globally</p>
              <p className='text-xl flex items-center'>{viewabilityArray.map(option =>
              <button
                key={option}
                className={`flex-1 ml-0 py-1 border border-solid border-slate-300 rounded ${viewability === option.toLowerCase() ? 'bg-sky-500/75 hover:bg-sky-500/60 border-transparent' : 'bg-transparent hover:bg-transparent text-black'}`}
              ...
              <button
                key={mode}
                className={`flex-1 ml-0 py-1 border border-solid border-slate-300 rounded ${editability === mode.toLowerCase() ? viewability === 'public' ? 'bg-sky-500/75 hover:bg-sky-500/60 border-transparent' : 'bg-sky-500/50 hover:bg-sky-500/50 border-transparent' : 'bg-transparent hover:bg-transparent disabled:bg-slate-300/20 text-black disabled:text-gray/75'}`}
                onClick={() => setEditability(mode.toLowerCase())} disabled={viewability === 'private' ? true : false}>{mode}</button>
              )}</p>
              <hr className='my-3' />
              <button className='m-0 mt-2 bg-sky-500/10 hover:bg-sky-500/20 text-sky-500 border border-solid border-sky-400 active:bg-sky-500/40 rounded w-[99%]' onClick={() => navigator.clipboard.writeText(`openterminal.vercel.app/augmentive/note/${note._id}`)}><FontAwesomeIcon icon={faLink} /> Copy link</button>
            </div>
          </div>
        </div> }
      </>
